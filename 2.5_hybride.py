...
for chemin_fichier in glob.glob("corpus/*_clean.txt"):
    ...
    for i, c in enumerate(chunks):
        ...
        tous_ids.append(f"{nom_medicament}_{i}")
...
def recherche_hybride(question, k=3):
    ...
    top_k = sorted(scores_finaux.items(), key=lambda x: x[1], reverse=True)[:k]
    return [chunk_id for chunk_id, score in top_k]
...
if __name__ == "__main__":
    ...
    for q in questions:
        ids_trouves = recherche_hybride(q["question"], k=k)
